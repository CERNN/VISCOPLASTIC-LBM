    ...
    Macroscopics* const __restrict__ macr)
{
    ...
    dfloat vx_pc = particleCenters[idx].vel.x;
    dfloat vy_pc = particleCenters[idx].vel.y;
    dfloat vz_pc = particleCenters[idx].vel.z;
    ...
    // WAINE QUE ESCREVEU ISSO SÓ PRA COMPILAR, REVER
    dfloat dA = 1, drs= 1;
